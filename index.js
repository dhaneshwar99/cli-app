...
function parseExcelToJson(filePath) {
    ...
    var sheetName = workbook.SheetNames[0]; // Assuming the data is in the first sheet
...
function main() {
    try {
        var excelFilePath = './iw-tech-test-retailer-data.xlsx'; // Update with your file path
        var jsonOutputFilePath = 'output.json'; // Update with desired output file path
        var jsonData = parseExcelToJson(excelFilePath);
        var rectifiedData = [];
        ...
        // console.log(jsonOutputFilePath)
    }
    catch (error) {
        console.error('Error:');
    }
...
function rectifyPhoneNumber(phoneNumber) {
    ...
    var phoneRegex = /^\d{10,15}$/; // Adjust the regex based on your requirements
    if (!phoneRegex.test(cleanedPhoneNumber)) {
        console.log('Invalid phone number format');
        return null; // Return null for invalid phone numbers
    }
    // Format the phone number (adjust as per your requirements)
    var formattedPhoneNumber = "".concat(cleanedPhoneNumber.slice(0, 2)).concat(cleanedPhoneNumber.slice(2, 5)).concat(cleanedPhoneNumber.slice(5, 8)).concat(cleanedPhoneNumber.slice(8));
...
function rectifyNumericId(id) {
    // Remove non-digit characters
    var cleanedId = id.toString().replace(/\D/g, '');
    ...
    var rectifiedId = String(Number(cleanedId));
    return rectifiedId;
}
function rectifyLatitude(latitude) {
    // Remove non-digit characters and trim whitespace
    var cleanedLatitude = latitude.toString().replace(/[^\d.-]/g, '').trim();
    ...
    // Convert to a number
    var numericLatitude = parseFloat(cleanedLatitude);
    ...
    var roundedLatitude = numericLatitude.toFixed(6);
    return roundedLatitude;
}
function rectifyLongitude(longitude) {
    // Remove non-digit characters and trim whitespace
    var cleanedLongitude = longitude.toString().replace(/[^\d.-]/g, '').trim();
    ...
    // Convert to a number
    var numericLongitude = parseFloat(cleanedLongitude);
    ...
    // Round to a reasonable number of decimal places (adjust as needed)
    var roundedLongitude = numericLongitude.toFixed(6);
